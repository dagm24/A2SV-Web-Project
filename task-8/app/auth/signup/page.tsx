...
export default function SignupPage() {
  ...
  const [state, formAction] = useActionState(
    async (prevState: unknown, formData: FormData) => {
      ...
      // If signup is successful, redirect on client
      if (result?.success) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-white">
      ...
      <div className="w-full max-w-md p-8 shadow-lg rounded-lg border bg-card text-card-foreground">
        ...
        <div className="p-6 pt-0">
          ...
          <button
            type="button"
            className="flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-10 px-4 w-full gap-2 text-gray-700 border-gray-300 hover:bg-gray-50 bg-transparent"
          >
          ...
          <form action={formAction} className="space-y-4">
            ...
            <div className="mt-4 text-center text-sm text-gray-600">
              ...
              <Link href="/auth/login" className="font-medium text-v0-purple hover:underline">
                Login
              </Link>
            </div>
            <p className="text-center text-xs text-gray-500 mt-4">
              By clicking &apos;Continue&apos;, you acknowledge that you have read and accepted our{" "}
              <Link href="#" className="font-medium text-v0-purple hover:underline">
                Terms of Service
              </Link>{" "}
              and{" "}
              <Link href="#" className="font-medium text-v0-purple hover:underline">
...
function SignupButton() {
  ...
  return (
    <button type="submit" className="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-600 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 h-10 px-4 py-2 w-full bg-purple-600 text-white hover:bg-purple-700" aria-disabled={pending}>
